...
class Range:
    ...
    def __set__(self, instance, value):
        ...
        if not all(i.isalpha() for i in value.split())::
            raise TypeError(f'Значение  {value} не должно содержать символы, кроме букв')
...
class Student:
    ...
    def __str__(self):
        return f'Student(name={self.first_name}, last name={self.last_name})'


if __name__ == '__main__':
    std_one = Student('maria', 'hоссум5')
    print(std_one)
